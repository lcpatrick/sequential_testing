import numpy as np
from scipy import stats

class AlphaFunctionAdjustmentTests(object):
    """
    Provide adjustments to critical z values and p values so that overall Type I error is respected, and Type I error follows a specified path, if given.
    """

    def __init__(
        self,
        setting,
        adjustment_type='constant_critical_value',
        alpha=0.05,
        num_tests=None,
        alpha_function=None,
        mean=None,
        sigma_m=None,
        z_simulation_runs=10000,
        metric_simulation_runs=1000
    ):
        """
        -- setting: 'sequential' or 'mht' (multiple hypothesis testing)
        -- adjustment_type: 'alpha_function' or 'constant_critical_value'
        -- alpha: total Type I error
        -- num_tests: total number of tests
        -- alpha_function: the schedule of how much total alpha can be spent, as a increasing numpy array
        -- mean: mean of the metrics, applicable only for 'mht' setting
        -- sigma_m: covariance of the metrics, applicable only for the 'mht' setting
        -- z_simulation_runs: number of simulations of z statistics in computing critical values
        -- metric_simulation_runs: number of simulations of realizations of metrics, applicable only for the 'mht' setting
        """
        self.setting = setting
        self.adjustment_type = adjustment_type
        self.alpha = alpha
        self.num_tests = num_tests
        self.alpha_function = alpha_function
        self.mean = mean
        self.sigma_m = sigma_m
        self.z_simulation_runs = z_simulation_runs
        self.metric_simulation_runs = metric_simulation_runs
        # check that num_tests and alpha_function are in agreement
        if num_tests and alpha_function:
            if num_tests != alpha_function.shape[0]:
                raise ValueError("Number of tests is inconsistent with dimension of alpha function.")
        if alpha_function:
            if alpha != alpha_function[-1]:
                raise ValueError("Total Type I error is inconsistent with alpha function.")
        # check that if setting 'mht' is used, then mean and sigma_m must be specified and are consistent
        # in dimensions
        if setting == 'mht':
            if np.any(mean == None) or np.any(sigma_m == None):
                raise ValueError("Metric means and covariances must be provided for multiple hypothesis testing adjustments.")
            if np.any(mean.shape[0] != sigma_m.shape[0]):
                raise ValueError("Dimensions of metric means and variances are inconsistent.")

    def simulate_alpha_sequential_constant_critical_value(self):
        Z_statistics = np.zeros((self.z_simulation_runs, self.num_tests)) # to store the z_statistics
        for sim in range(sims):
            control_data = np.random.normal(mean, std, M * K)
            treatment_data = np.random.normal(mean, std, M * K)
            for k in range(K):
                control_data_up_to_period_k = control_data[:M * (k + 1)]
                treatment_data_up_to_period_k = treatment_data[:M * (k + 1)]
                diff = control_data_up_to_period_k.mean() - treatment_data_up_to_period_k.mean()
                diff_std = np.sqrt(
                    control_data_up_to_period_k.var() / (M * (k + 1)) +
                    control_data_up_to_period_k.var() / (M * (k + 1))
                )
                z = diff / diff_std
                Z_statistics[sim, k] = z
        # compute the false positive rate using Bonferonni critical value
        bonferonni_critical_value = 2.80
        (np.max(np.abs(Z_statistics), axis=1) > bonferonni_critical_value).mean()

    def simulate_alpha_mht_constant_critical_value(self):
        pass


import numpy as np
from scipy import stats

class MHT(object):
    def __init__(self, alpha, mean, sigma_m, sims=100000, num_units=1000):
        super().__init__()
        self.alpha = alpha # Type I error
        self.mean = mean # mean of the metrics, numpy array
        self.sigma_m = sigma_m # covariance matrix of the metrics, numpy array
        self.sims = sims # number of simulations
        self.num_units = num_units # number of simulated units in each variant

    def _get_sigma_z(self):
        # compute the covariance matrix of the z_statistics
        K = self.sigma_m.shape[0] # number of metrics
        sigma_z = np.ones((K, K))
        for i in range(K):
            for j in range(K):
                sigma_z[i, j] = sigma_m[i, j] / np.sqrt(sigma_m[i, i] * sigma_m[j, j])
        return sigma_z

    def compute_constant_p_value(self):
        sigma_z = self._get_sigma_z()
        K = sigma_z.shape[0] # number of metrics
        Z_statistics = np.zeros((self.sims, K))
        z_mean = np.zeros((K))
        Z = np.random.multivariate_normal(z_mean, sigma_z, self.sims)
        Z_abs = np.max(np.abs(Z), axis=1)
        z_c = np.percentile(Z_abs, 100 * (1 - self.alpha))
        p_value = 2 * (1 - stats.norm.cdf(z_c, loc=0, scale=1))
        print("adjusted p_value is {}".format(p_value))
        print("adjusted critical z value is {}".format(z_c))
        return (z_c, p_value)

    def compute_overall_reject_freq(self, critical_value):
        """
        computes the overall Type I error, given a critical value
        critical_value: the value to determine rejection
        """
        K = sigma_m.shape[0] # number of metrics
        N = self.num_units # number of units in each variant
        Z = np.zeros((self.sims, K))
        for sim in range(self.sims):
            # simulate data, and compute z_statistics for each of the K metrics
            data_A = np.random.multivariate_normal(mean, self.sigma_m, N)
            data_B = np.random.multivariate_normal(mean, self.sigma_m, N)
            # compute z_statistic for test on each metric, and store
            for metric in range(K):
                delta = data_A[:, metric].mean() - data_B[:, metric].mean()
                std = np.sqrt(
                    data_A[:, metric].var() / N +
                    data_A[:, metric].var() / N)
                z = delta / std
                Z[sim, metric] = z

        # count frequency of rejecting null for at least one metric
        reject_freq = (np.max(np.abs(Z), axis=1) > critical_value).mean()
        print("rejection frequency is {}".format(reject_freq))

    def compute_alpha_expenditure(self, critical_value):
        """
        computes how much of Type I error each test incurs
        """
        sigma_z = self._get_sigma_z()
        K = sigma_z.shape[0] # number of metrics
        alphas = np.zeros((K,)) # store of Type I error incurred
        Z_statistics = np.zeros((self.sims, K))
        z_mean = np.zeros((K))
        Z = np.random.multivariate_normal(z_mean, sigma_z, self.sims)
        for i in range(K):
            Z_abs = np.max(np.abs(Z[:, :i+1]), axis=1)
            alpha = (Z_abs > critical_value).mean()
            alphas[i] = alpha
        return alphas